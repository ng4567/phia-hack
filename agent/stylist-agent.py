...
PHOEBE_GREETING = "Hi Phoebe, how may I help you"
...
def _is_start_message(message: str) -> bool:
    ...
    return normalized in {"/start", "start", "hi", "hello", "hey"}


def _is_event_question(message: str) -> bool:
    lowered = message.lower()
    return any(keyword in lowered for keyword in ("event", "calendar", "upcoming", "schedule"))


def _is_style_request(message: str) -> bool:
    lowered = message.lower()
    return any(
        keyword in lowered
        for keyword in (
...
def _events_text(events: Sequence[CalEvent]) -> str:
    if not events:
        return "I do not see any upcoming events in your Google Calendar right now."

    lines = ["Here are your upcoming events:"]
    for event in events:
        lines.append(
            f"- {event.title} on {event.start.strftime('%a, %b %d %I:%M %p')} at {event.location}."
        )
    return "\n".join(lines)


async def simulate_telegram_chat_with_phoebe(
    user_message: str,
    *,
    person_image_uri: str = "placeholder://person-image",
    person_image_metadata: dict[str, Any] | None = None,
) -> dict[str, Any]:
    message = user_message.strip()
    if not message or _is_start_message(message):
        return {
            "reply": PHOEBE_GREETING,
            "intent": "greeting",
            "events": [],
            "products_by_event": [],
            "all_products": [],
    ...
    events = get_user_events("google_calendar")

    if _is_event_question(message):
        return {
            "reply": _events_text(events),
            "intent": "events",
            "events": [
                {
                    "id": event.id,
    ...
    if _is_style_request(message):
        products = search_products(events)
        design_seed = make_design(
            products,
            person_image=Image(
                uri=person_image_uri,
                metadata=person_image_metadata or {"client_id": "phoebe"},
            ),
        )
    ...
    return {
        "reply": (
            f"{PHOEBE_GREETING} You can ask 'what events do I have' or "
            "'generate styles for my upcoming events'."
        ),
        "intent": "fallback",
        "events": [],
        "products_by_event": [],
